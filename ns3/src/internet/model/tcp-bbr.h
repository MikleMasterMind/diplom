...
class TcpBbr : public TcpCongestionOps
{
  ...
  public:
    ...
    /**
     * @brief Gets the max bandwidth filter.
     * @return returns the max bandwidth filter.
     */
    const MaxBandwidthFilter_t& GetMaxBwFilter() const;

    /**
     * @brief Gets the current minimum RTT (RTprop).
     * @return returns the current minimum RTT.
     */
    Time GetMinRtt() const;
  ...
  private:
    ...
    TracedValue<Time> m_rtpropRaw{Time::Max()}; //!< Raw RTT sample considered for RTprop update.
    TracedValue<uint64_t> m_btlBwRaw{0}; //!< Raw delivery-rate sample considered for BtlBw update.
