...
TcpSocketBase::DupAck(uint32_t currentDelivered)
{
    ...
    const uint32_t dupAckThresh = GetEffectiveDupAckThresh();
    m_txBuffer->SetDupAckThresh(dupAckThresh);
#endif
    ...
    {
#ifdef TCP_SOCKET_BASE_USE_NEW_DUPACK_LOGIC
        // Remove the strict assertion - with dynamic dupAck threshold, we can exceed it
        // during packet reordering scenarios. The important thing is to transition to
...
#ifdef TCP_SOCKET_BASE_USE_NEW_DUPACK_LOGIC
        // Modified condition: Always enter recovery if dupAckCount exceeds threshold,
        // regardless of m_recoverActive state. This prevents the assertion failure
        // when packets are reordered across multiple paths.
...
uint32_t
TcpSocketBase::GetEffectiveDupAckThresh() const
{
    // По умолчанию используем обычный статический порог
    ...
    {
        return std::max(1u, m_retxThresh);
    }

    return std::max(1u,
                    static_cast<uint32_t>((m_tcb->m_cWnd.Get() + m_tcb->m_segmentSize - 1) /
                                          m_tcb->m_segmentSize));
}

void
TcpSocketBase::SyncDupAckThresh()
{
    const uint32_t dupAckThresh = GetEffectiveDupAckThresh();
    m_txBuffer->SetDupAckThresh(dupAckThresh);

    NS_LOG_DEBUG("Effective dupAck threshold = "
                 << dupAckThresh << " for CC = "
                 << (m_congestionControl ? m_congestionControl->GetName() : "null"));
}
